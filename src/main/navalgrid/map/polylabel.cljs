(ns navalgrid.map.polylabel
  (:require [navalgrid.math :as math]))

(defn bounding-box
  "Returns the bounding box of the specified polygon (using lnglats)"
  [polygon]
...
(defn pole-of-inaccessability
  "Returns the pole of inaccessibility (as lnglat) for the specified polygon (using lnglats) with the specified precision"
  ([polygon] (pole-of-inaccessability polygon 1.0))
  ([polygon precision]
   (let [{min-x :min-x
          min-y :min-y
          max-x :max-x
